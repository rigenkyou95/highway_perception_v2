...
class CameraConfig:
    ...
    # 可选：畸变参数（当前脚本未做去畸变，只做记录）
    dist: Optional[List[float]] = None  # e.g. [k1,k2,p1,p2,k3]

    # ======= 外参（地面坐标系简化：只用高度 + 姿态）=======
    cam_height: float = 1.4      # 相机离地高度（米）
    pitch_deg: float = 0.0       # 俯仰角：向下为正（度）
    roll_deg: float = 0.0        # 横滚角：右侧下沉为正（度）【当前未使用】
    yaw_deg: float = 0.0         # 偏航角：向右为正（度）【当前未使用】

    # ======= 兜底参数（当缺少真实 fx/fy 时）=======
    fov_deg: float = 90.0        # 水平视场角（度），仅用于无标定时估 fx/fy
...
# ============================================================
# 相机 YAML 配置：读取真实标定参数（intrinsic / extrinsic）
# ============================================================
...
def load_camera_from_yaml(cam_yaml: Optional[str], img_wh: Tuple[int, int]) -> bool:
    ...
    if yaml is None:
        raise RuntimeError(
            "[Camera] 需要读取 YAML，但未安装 pyyaml。请先执行: pip install pyyaml"
        )
    ...
    if p.is_dir():
        ...
        if not intr_path.is_file():
            raise FileNotFoundError(f"[Camera] 目录模式缺少 intrinsics.yaml: {intr_path}")
        if not extr_path.is_file():
            raise FileNotFoundError(f"[Camera] 目录模式缺少 extrinsics.yaml: {extr_path}")

        intr_data = yaml.safe_load(intr_path.read_text(encoding="utf-8"))
        extr_data = yaml.safe_load(extr_path.read_text(encoding="utf-8"))

        if not isinstance(intr_data, dict):
            raise ValueError(f"[Camera] intrinsics.yaml 内容不是 dict: {intr_path}")
        if not isinstance(extr_data, dict):
            raise ValueError(f"[Camera] extrinsics.yaml 内容不是 dict: {extr_path}")
    ...
    # --------------------------------------------------
        ...
        if not isinstance(data, dict):
            raise ValueError(f"[Camera] YAML 内容不是 dict: {p}")

        intr = data.get("intrinsic", {}) or {}
        ext = data.get("extrinsic", {}) or {}

        print(f"[Camera] Loaded YAML file: {p}")
    ...
    if "dist" in intr and intr["dist"] is not None:
        if isinstance(intr["dist"], (list, tuple)):
            cfg.camera.dist = [float(x) for x in intr["dist"]]
        else:
    ...
    if "pitch_deg" in ext and ext["pitch_deg"] is not None:
        cfg.camera.pitch_deg = float(ext["pitch_deg"])
    if "roll_deg" in ext and ext["roll_deg"] is not None:
        cfg.camera.roll_deg = float(ext["roll_deg"])
    ...
    print(f"[Camera]   H={cfg.camera.cam_height:.3f} m, "
          f"pitch={cfg.camera.pitch_deg:.3f}°, "
          f"roll={cfg.camera.roll_deg:.3f}°, "
          f"yaw={cfg.camera.yaw_deg:.3f}°")

    if cfg.camera.cx <= 0 or cfg.camera.cy <= 0:
        print("[Camera][Warn] cx/cy ≤ 0，内参可能无效。")

    if abs(cfg.camera.cx - w0 / 2.0) > 0.15 * w0 or abs(cfg.camera.cy - h0 / 2.0) > 0.15 * h0:
        print("[Camera][Warn] 主点(cx,cy)与图像中心偏差较大。若有裁剪/缩放，请确认内参已同步。")

    return True


def ensure_camera_intrinsics(img_wh: Tuple[int, int]):
    """若未提供真实 fx/fy，则用 FOV 兜底估计。"""
    w0, h0 = img_wh
    if cfg.camera.fx > 1e-6 and cfg.camera.fy > 1e-6 and cfg.camera.cx > 1e-6 and cfg.camera.cy > 1e-6:
        return

    print("[Camera] 未提供完整真实内参，使用 FOV 近似计算 fx, fy, cx, cy（仅兜底）")
    cfg.camera.cx = w0 / 2.0
    cfg.camera.cy = h0 / 2.0
    fx_fy = w0 / (2.0 * math.tan(math.radians(cfg.camera.fov_deg) / 2.0))
    cfg.camera.fx = fx_fy
    cfg.camera.fy = fx_fy
    print(f"[Camera]   fx=fy={fx_fy:.2f}, cx={cfg.camera.cx:.1f}, cy={cfg.camera.cy:.1f}")


def evaluate_lane_width_consistency(
    ll_mask_pitch_255: np.ndarray,
    cam_cfg: CameraConfig,
    lane_width_m: float = 3.6
) -> dict:
    """
    用“车道宽约 3.6m”对当前相机(H, pitch, fx, fy, cx, cy)做一致性评估（不做优化，只报误差）。
    返回指标 dict，并在控制台打印摘要。
    """
    # ll_mask_pitch_255: 0/255 的中心线 mask
    ys_arr, dx_obs = collect_lane_width_samples(ll_mask_pitch_255, cam_cfg)
    if ys_arr.size == 0:
        print("[Check] 无法收集车道宽样本：lane width consistency 跳过。")
        return {"ok": False, "reason": "no_samples"}

    fx, fy, cy, H = cam_cfg.fx, cam_cfg.fy, cam_cfg.cy, cam_cfg.cam_height
    theta = math.radians(cam_cfg.pitch_deg)

    Yc = (ys_arr - cy) / (fy + 1e-6) + math.tan(theta)
    valid = Yc > 1e-4
    ys_arr = ys_arr[valid]
    dx_obs = dx_obs[valid]
    if ys_arr.size == 0:
        print("[Check] 样本全部位于地平线以上/无效，跳过。")
        return {"ok": False, "reason": "invalid_samples"}

    Z = H / (Yc + 1e-6)
    dx_pred = fx * lane_width_m / (Z + 1e-6)

    abs_err = np.abs(dx_pred - dx_obs)
    med_err = float(np.median(abs_err))
    p90_err = float(np.percentile(abs_err, 90))
    med_obs = float(np.median(dx_obs))
    med_pred = float(np.median(dx_pred))

    print("[Check] Lane-width consistency (3.6m):")
    print(f"[Check]   samples={ys_arr.size}, dx_obs_median={med_obs:.2f}px, dx_pred_median={med_pred:.2f}px")
    print(f"[Check]   |dx_pred-dx_obs| median={med_err:.2f}px, p90={p90_err:.2f}px")

    # 给一个很直观的“是否可用”判断阈值（可按你的分辨率再调）
    ok = med_err < 25.0
    if not ok:
        print("[Check][Warn] 误差偏大：通常意味着 pitch 或 height 或 fx/fy 与当前图像分辨率/裁剪不一致。")

    return {
        "ok": ok,
        "samples": int(ys_arr.size),
        "dx_obs_median_px": med_obs,
        "dx_pred_median_px": med_pred,
        "abs_err_median_px": med_err,
        "abs_err_p90_px": p90_err,
    }
...
def run_full_pipeline(img_path: str, cam_yaml: Optional[str] = None):
    ensure_dirs()
    device = cfg.device
    print("ROOT   :", ROOT)
    print("Device :", device)
    ...
    ensure_camera_intrinsics(img_wh=(w0, h0))
    print(f"[Camera] fx={cfg.camera.fx:.2f}, fy={cfg.camera.fy:.2f}, cx={cfg.camera.cx:.1f}, cy={cfg.camera.cy:.1f}, H={cfg.camera.cam_height:.3f}, pitch={cfg.camera.pitch_deg:.3f}°")
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="相机真实标定参数 YAML 路径（推荐）。若不提供则使用 FOV 兜底估计内参。",
    )
...
if __name__ == "__main__":
    args = parse_args()
    run_full_pipeline(args.img, cam_yaml=args.cam_yaml)
