...
def summarize_one(csv_path: Path, range_name: str):
    ...
    s_final_valid = s_final.dropna()

    if len(s_final_valid) >= 2:
        delta_s = s_final_valid.diff().abs().dropna()
    else:
        delta_s = pd.Series(dtype=float)
    ...
    return {
        "anchor_range": range_name,
        "frames": len(df),
        "candidate_valid_ratio": candidate_valid_ratio,
        "tracking_ratio": tracking_ratio,
        "hold_ratio": hold_ratio,
        "mean_anchors": mean_anchors,
        "mean_s_final": s_final_valid.mean(),
        "std_s_final": s_final_valid.std(),
...
def main():
    ...
    for range_name, folder in RANGE_DIRS.items():
        csv_path = root_dir / folder / "runtime_per_frame.csv"

        if not csv_path.exists():
            raise FileNotFoundError(f"Missing CSV for {range_name}: {csv_path}")

        print(f"[INFO] Processing {range_name}: {csv_path}")
        rows.append(summarize_one(csv_path, range_name))
    ...
    print("\n=== Anchor Range Sensitivity Summary ===")
